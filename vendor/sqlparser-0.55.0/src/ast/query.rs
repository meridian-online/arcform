...
pub enum SetExpr {
    ...
    Update(Statement),
    Table(Box<Table>),
    /// DuckDB "simplified" statement-form `PIVOT` (`PIVOT t ON … USING …`),
    /// usable wherever a query body is: as a top-level statement, a CTAS body,
    /// or a subquery. Distinct from the SQL-standard `FROM t PIVOT (…)`
    /// table-factor form, which is [`TableFactor::Pivot`].
    Pivot(Box<PivotStatement>),
    /// DuckDB "simplified" statement-form `UNPIVOT`; the counterpart to
    /// [`SetExpr::Pivot`], distinct from [`TableFactor::Unpivot`].
    Unpivot(Box<UnpivotStatement>),
}
...
impl fmt::Display for SetExpr {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            SetExpr::Pivot(p) => write!(f, "{p}"),
            SetExpr::Unpivot(u) => write!(f, "{u}"),
...
/// DuckDB statement-form `PIVOT`:
/// `PIVOT <source> ON <expr>[, …] [USING <aggregate>[, …]] [GROUP BY <expr>[, …]]`
///
/// DuckDB's "simplified" pivot. Unlike the SQL-standard [`TableFactor::Pivot`]
/// (`FROM t PIVOT (…)`), this is a query in its own right — a top-level
/// statement, a CTAS body, or a subquery — and auto-detects the pivot values
/// from the data rather than requiring an explicit `IN (…)` list.
/// See <https://duckdb.org/docs/stable/sql/statements/pivot>.
#[derive(Debug, Clone, PartialEq, PartialOrd, Eq, Ord, Hash)]
#[cfg_attr(feature = "serde", derive(Serialize, Deserialize))]
#[cfg_attr(feature = "visitor", derive(Visit, VisitMut))]
pub struct PivotStatement {
    /// The relation being pivoted (table name, table function, or subquery).
    pub source: Box<TableFactor>,
    /// `ON` — the column expressions whose distinct values become output columns.
    pub on: Vec<Expr>,
    /// `USING` — the aggregate expressions computed per pivoted cell. Empty when
    /// omitted (DuckDB then defaults to a `COUNT`).
    pub using: Vec<ExprWithAlias>,
    /// `GROUP BY` — the columns retained as rows. Empty when omitted.
    pub group_by: Vec<Expr>,
}

impl fmt::Display for PivotStatement {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "PIVOT {} ON {}", self.source, display_comma_separated(&self.on))?;
...
/// DuckDB statement-form `UNPIVOT`:
/// `UNPIVOT <source> ON <expr>[, …] [INTO NAME <ident> VALUE <ident>[, …]]`
///
/// The query counterpart to [`TableFactor::Unpivot`]; see [`PivotStatement`].
/// <https://duckdb.org/docs/stable/sql/statements/unpivot>.
#[derive(Debug, Clone, PartialEq, PartialOrd, Eq, Ord, Hash)]
#[cfg_attr(feature = "serde", derive(Serialize, Deserialize))]
#[cfg_attr(feature = "visitor", derive(Visit, VisitMut))]
pub struct UnpivotStatement {
    /// The relation being unpivoted (table name, table function, or subquery).
    pub source: Box<TableFactor>,
    /// `ON` — the columns collapsed into name/value rows.
    pub on: Vec<Expr>,
    /// `INTO NAME … VALUE …` — the name/value output columns. `None` uses
    /// DuckDB's defaults (`name`/`value`).
    pub into: Option<UnpivotInto>,
}

/// The `INTO NAME <ident> VALUE <ident>[, …]` clause of an [`UnpivotStatement`].
#[derive(Debug, Clone, PartialEq, PartialOrd, Eq, Ord, Hash)]
#[cfg_attr(feature = "serde", derive(Serialize, Deserialize))]
#[cfg_attr(feature = "visitor", derive(Visit, VisitMut))]
pub struct UnpivotInto {
    /// `NAME <ident>` — the column holding the former column names.
    pub name: Ident,
    /// `VALUE <ident>[, …]` — the column(s) holding the former column values.
    pub value: Vec<Ident>,
}

impl fmt::Display for UnpivotStatement {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "UNPIVOT {} ON {}", self.source, display_comma_separated(&self.on))?;
        if let Some(into) = &self.into {
            write!(
                f,
                " INTO NAME {} VALUE {}",
                into.name,
                display_comma_separated(&into.value)
            )?;
        }
        Ok(())
    }
