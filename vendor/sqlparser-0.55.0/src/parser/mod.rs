...
impl<'a> Parser<'a> {
    ...
    pub fn parse_statement(&mut self) -> Result<Statement, ParserError> {
        ...
        match &next_token.token {
            Token::Word(w) => match w.keyword {
                ...
                // DuckDB statement-form PIVOT/UNPIVOT (`PIVOT t ON … USING …`) are
                // queries in their own right — route them through `parse_query`,
                // which handles the body via `parse_query_body`.
                Keyword::PIVOT | Keyword::UNPIVOT
    ...
    fn parse_copy_option(&mut self) -> Result<CopyOption, ParserError> {
        let ret = match self.parse_one_of_keywords(&[
            ...
            _ => {
                // DuckDB's COPY accepts an open-ended set of `name [value]` options
                // (COMPRESSION, PARTITION_BY, ROW_GROUP_SIZE, OVERWRITE_OR_IGNORE, …)
                // beyond the fixed PostgreSQL keywords above. Capture them generically
                // so multi-option COPY parses instead of degrading to an opaque step.
                if dialect_of!(self is DuckDbDialect | GenericDialect) {
                    let name = self.parse_identifier()?;
                    ...
                    self.expected("option", self.peek_token())?
                }
            }
        };
        Ok(ret)
    ...
    pub fn parse_query_body(&mut self, precedence: u8) -> Result<Box<SetExpr>, ParserError> {
        ...
        } else if self.peek_keyword(Keyword::PIVOT)
            && dialect_of!(self is DuckDbDialect | GenericDialect)
        {
            // DuckDB statement-form PIVOT as a query body (top-level, CTAS body,
            // or subquery). See `parse_pivot_statement`.
            SetExpr::Pivot(Box::new(self.parse_pivot_statement()?))
        } else if self.peek_keyword(Keyword::UNPIVOT)
            && dialect_of!(self is DuckDbDialect | GenericDialect)
        {
            SetExpr::Unpivot(Box::new(self.parse_unpivot_statement()?))
        } else {
            return self.expected(
    ...
    /// Parse DuckDB's statement-form `PIVOT`:
    /// `PIVOT <source> ON <expr>[, …] [USING <aggregate>[, …]] [GROUP BY <expr>[, …]]`.
    ///
    /// The `PIVOT` keyword is at the cursor. Distinct from
    /// [`Self::parse_pivot_table_factor`], the SQL-standard `FROM t PIVOT (…)` form:
    /// this variant is a query in its own right (top-level, CTAS body, subquery) and
    /// auto-detects the pivot values, so there is no explicit `IN (…)` list.
    pub fn parse_pivot_statement(&mut self) -> Result<PivotStatement, ParserError> {
        self.expect_keyword_is(Keyword::PIVOT)?;
        // `ON` is reserved for a table alias, so `parse_table_factor` stops before it.
    ...
    /// Parse DuckDB's statement-form `UNPIVOT`:
    /// `UNPIVOT <source> ON <expr>[, …] [INTO NAME <ident> VALUE <ident>[, …]]`.
    ///
    /// The `UNPIVOT` keyword is at the cursor. The counterpart to
    /// [`Self::parse_pivot_statement`]; distinct from
    /// [`Self::parse_unpivot_table_factor`].
    pub fn parse_unpivot_statement(&mut self) -> Result<UnpivotStatement, ParserError> {
        ...
        let into = if self.parse_keyword(Keyword::INTO) {
            ...
            let value = self.parse_comma_separated(Parser::parse_identifier)?;
            Some(UnpivotInto { name, value })
        } else {
            None
        };
        Ok(UnpivotStatement { source, on, into })
    }
